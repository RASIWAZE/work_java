...
public class Lesson08 {
	public static void main(String[] args) {
		...
		//mean()メソッドを呼び出して1, 2, 4の引数を渡して戻り値をdouble型の変数resultに代入
		double result = method.mean(1, 2, 4);
...
//		あり
	...
	//３つの数値を引数として受け取り、その平均値を返すメソッド
	public double mean(int x, int y, int z) {
		
		//小数点第一位までを表示させるようにしています
		return Math.round(((x + y + z) / 3.0) * 10.0) / 10.0;
	}
	
	//引数が３つでない場合や、引数が数値でない場合はコンパイルエラーになり処理が行われません
